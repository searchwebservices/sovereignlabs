...
export async function GET(
  ...
  try {
    ...
    const device = await serverDevicesApi.getById(id);
    return apiSuccess(device);
  } catch (error: unknown) {
    const err = error as { code?: string };
    if (err?.code === "PGRST116") return apiNotFound("Device");
...
export async function PATCH(
  ...
  try {
    ...
    const device = await serverDevicesApi.update(id, body);
    return apiSuccess(device);
  } catch (error: unknown) {
    const err = error as { code?: string };
    if (err?.code === "PGRST116") return apiNotFound("Device");
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await serverDevicesApi.delete(id);
