...
export async function GET(
  ...
  try {
    ...
    const part = await serverPartsApi.getById(id);
    return apiSuccess(part);
  } catch (error: unknown) {
    const err = error as { code?: string };
    if (err?.code === "PGRST116") return apiNotFound("Part");
...
export async function PATCH(
  ...
  try {
    ...
    const part = await serverPartsApi.update(id, body);
    return apiSuccess(part);
  } catch (error: unknown) {
    const err = error as { code?: string };
    if (err?.code === "PGRST116") return apiNotFound("Part");
...
export async function DELETE(
  ...
  try {
    const { id } = await params;
    await serverPartsApi.delete(id);
